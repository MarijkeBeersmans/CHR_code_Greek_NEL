...
def load_entity_dict(logger, params, is_zeshel, is_kurz):
    if is_zeshel:
        return load_entity_dict_zeshel(logger, params)
    if is_kurz:
        path = '/home/pricie/marijkeb/notebooks/CHR_NEL_code/Paulys_kb/blink_real_dict_v1_Kurz.jsonl'
    else:
        path = '/home/pricie/marijkeb/notebooks/CHR_NEL_code/Paulys_kb/blink_real_dict_v1_Voll.jsonl'
...
def load_or_generate_candidate_pool(
    ...
    is_kurz = params.get("is_kurz", None)
    ...
    if candidate_pool is None:
        ...
        entity_desc_list = load_entity_dict(logger, params, is_zeshel, is_kurz)
        logging.info('first few entities loaded using')
...
def main(params):
    ...
    logging.info(f"""as saved for the cross_encoder to train on data\n context_vecs:{new_data["context_vecs"][0]}
                    correct candidate_vec: {new_data["candidate_vecs"][0][new_data["labels"][0]]}
                    first candidateO_vec: {new_data["candidate_vecs"][0][0]}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_kurz', action='store_true')
    ...
    for mode in mode_list:
        ...
        new_params["is_kurz"] = args.is_kurz
        main(new_params)
