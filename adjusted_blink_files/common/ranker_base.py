...
class XLMRobertaEncoder(nn.Module):
    def __init__(self, xlm_roberta_model, output_dim, layer_pulled=-1, add_linear=None):
        super(XLMRobertaEncoder, self).__init__()
        self.layer_pulled = layer_pulled
        roberta_output_dim = xlm_roberta_model.embeddings.word_embeddings.weight.size(1)

        self.xlm_roberta_model = xlm_roberta_model
        if add_linear:
            self.additional_linear = nn.Linear(roberta_output_dim, output_dim)
            self.dropout = nn.Dropout(0.1)
        else:
            self.additional_linear = None

    def forward(self, token_ids, attention_mask):
        outputs = self.xlm_roberta_model(input_ids=token_ids, attention_mask=attention_mask, return_dict=True)
